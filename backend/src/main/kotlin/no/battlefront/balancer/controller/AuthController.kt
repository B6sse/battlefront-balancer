...
class AuthController(
    ...
    fun login(@RequestBody request: LoginRequest, httpRequest: HttpServletRequest): ResponseEntity<Any> {
        ...
        val role = details.authorities.firstOrNull()?.authority?.removePrefix("ROLE_") ?: ""
     ...
     */
    @GetMapping("/me")
    fun me(@AuthenticationPrincipal principal: AppUserDetails?): ResponseEntity<CurrentUserDto> {
        if (principal == null) return ResponseEntity.status(401).build()
        val role = principal.authorities.firstOrNull()?.authority?.removePrefix("ROLE_") ?: ""
